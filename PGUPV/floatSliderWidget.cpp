...
FloatSliderWidget::FloatSliderWidget(const std::string &label, float value, float v_min, float v_max) : 
    min(v_min), max(v_max), displayFormat("%.3f"), power(1.0f), value(value)
{
    setLabel(label);
};
...
void FloatSliderWidget::setPower(float sliderPower)
{
    power = sliderPower;
}
...
void FloatSliderWidget::renderWidget() 
{
    ...
    if (GUILib::SliderFloat(label, f, min, max, displayFormat, power))
        value.setValue(f);
}
...
float FloatSliderWidget::getPower() const { 
    return power; 
};
