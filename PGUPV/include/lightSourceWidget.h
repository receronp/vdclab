...
namespace PGUPV {
  ...
  protected:
    ...
    void build() {
      ...
      ambientCompSelector = std::make_shared<RGBAColorWidget>("Ambiental");
      ambientCompSelector->getValue().addListener(vec4UBOUpdater);
      diffuseCompSelector = std::make_shared<RGBAColorWidget>("Difusa");
      diffuseCompSelector->getValue().addListener(vec4UBOUpdater);
      specularCompSelector = std::make_shared<RGBAColorWidget>("Especular");
